...
const REG_ADDR: u8 = 0x3B; // ACCEL_XOUT_H start address
...
async fn main(spawner: Spawner) {
    ...
    let tx_reset = [!(1 << 7) & WRITE_ADDR_PWR, 0x80]; // 0x80 = Device Reset bit
    ...
    // --- STEP 1: CALIBRATE GYRO Z (Keep board still during boot) ---
    info!("Calibrating Gyro Z... Do not move board!");
    let mut gyro_z_sum: f32 = 0.0;
    let samples = 200;
    for _ in 0..samples {
        let tx_buf = [(1 << 7) | 0x47, 0x00, 0x00]; // Read GYRO_ZOUT_H & L (0x47 & 0x48)
